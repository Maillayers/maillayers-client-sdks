...
import type { MergeTagOption, StyleRecord, ThemeDefinition } from './types';
...
function validateStyle(value: unknown): StyleRecord {
  const snapshot = snapshotObject(value, {
    maxDepth: 1,
  ...
  return snapshot as StyleRecord;
}
...
export function validateEmailBuilderInputs(input: Record<string, unknown>): RuntimeInputValidation {
  try {
    ...
    let style: StyleRecord | undefined;
    try {
